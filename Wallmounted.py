...
def get_disp_component(selected_components):
    ...
    selected_models = set()
    required_keys = {"model", "pole", "min_current", "max_current"}
    ...
    return [{k: frame.get(k) for k in required_keys} for frame in Component_Frames if isinstance(frame, dict) and frame.get("model") in selected_models]
...
def main():
    ...
    if Outgoing_components:
            with st.expander(f"Components Details", expanded=False):
                data2 = get_disp_component(Outgoing_components)
                st.table(data2)
    else:
            st.info("Select a breaker.")
